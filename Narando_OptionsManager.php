...
public function settingsPage() {
  ...
  if (!current_user_can('manage_options')) {
    wp_die(__('You do not have sufficient permissions to access this page.', 'narando'));
  }
  ?>
  <div>
    ...
    // HTML for the page
    // wp-admin/plugins.php?page=Narando_PluginSettings
    $settingsGroup = get_class($this) . '-settings-group';
    ?>
    <div class="wrap">
      <h2><?php _e('System Settings', 'narando'); ?></h2>
      <table class="form-table"><tbody>
        <tr><td><?php _e('PHP Version', 'narando'); ?></td>
          <td><?php echo phpversion(); ?>
            <?php
            if (version_compare('5.2', phpversion()) > 0) {
              echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
              _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'narando');
              echo '</span>';
            }
            ?>
          </td>
        </tr>
        <tr><td><?php _e('MySQL Version', 'narando'); ?></td>
          <td><?php echo $this->getMySqlVersion() ?>
            <?php
            echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
            if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
              _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'narando');
            }
            echo '</span>';
            ?>
          </td>
    ...
    <div class="plugin_config">
      <div id="plugin_config_tabs">
        <ul>
          <li><a href="#plugin_config-1">Preferences</a></li>

        </ul>
        <div id="plugin_config-1">
          <?php $this->outputTabContentsGeneralSettings(); ?>
        </div>
...
<?php
*/
// narando Settings HTML part ?>
<?php /*
<h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'narando'); ?></h2>
...
protected function createFormControl($aOptionKey, $aOptionMeta, $savedOptionValue, $aOptionFormat = 'select') {
  ...
  if($aOptionKey == "NRPreText" || $aOptionKey == "NRPostText") {
    ...
    <?php
  } elseif ($aOptionKey == "NRCheckIfPlayerOrButtonIsSet") {
  ...
  */
  protected function getOptionValueI18nString($optionValue) {
    switch ($optionValue) {
      case 'true':
      return __('Yes', 'narando');
      case 'false':
      return __('No', 'narando');
      case 'Administrator':
      return __('Administrator', 'narando');
      case 'Editor':
      return __('Editor', 'narando');
      case 'Author':
      return __('Author', 'narando');
      case 'Contributor':
      return __('Contributor', 'narando');
      case 'Subscriber':
      return __('Subscriber', 'narando');
      case 'Anyone':
      return __('Anyone', 'narando');

      // set Player or Button
      case 'player':
      return __('Player', 'narando');
      case 'button':
      return __('Button', 'narando');
      case 'selection':
      return __('Selection', 'narando');

      // set Horizontal or Vertical
      case 'horizontal':
      return __('Horizontal', 'narando');
      case 'vertical':
      return __('Vertical', 'narando');
    }
    return $optionValue;
  }
